    ...
    LSC = Node(
        ...
        Mesh(
            ...
            material = Material(
                refractive_index = 1.5,
                components = [
                    Absorber(coefficient = 0.000000000000000000001),
                    ]
            ),
        ),
        parent = world
...
def analyzeResults(entrance_rays, exit_rays, exit_norms):
    ...
    print("\n Optical efficiency: " + str(edge_emit / numRays) + "\n")
    print("\t\tLeft \tRight \tFront \tBack \n")
    print("Edge emission\t" + str(edge_emit_left / numRays) + " \t" + str(edge_emit_right / numRays) + " \t" + str(
        edge_emit_front / numRays) + " \t" + str(edge_emit_back / numRays) + " \n")
    print("Bottom emission\t" + str(edge_emit_bottom / numRays) + "\t Absorption coeff " + str(
        -np.log10(edge_emit_bottom / numRays) / float(LSCdimZ)) + "\n")
    print("Top emission\t" + str(edge_emit_top / numRays) + "\n")
    if (saveFileName != ''):
        dataFile.write("Opt eff\t" + str(edge_emit / numRays) + "\n")
        dataFile.write("\t\tLeft \tRight \tFront \tBack \n")
        dataFile.write(
            "Edge emission\t" + str(edge_emit_left / numRays) + " \t" + str(edge_emit_right / numRays) + " \t" + str(
                edge_emit_front / numRays) + " \t" + str(edge_emit_back / numRays) + " \n")
        dataFile.write("type\tposx\tposy\tposz\tdirx\tdiry\tdirz\tsurfx\tsurfy\tsurfz\twav\n")
        dataFile.write("Bottom emission\t" + str(edge_emit_bottom / numRays) + "\t Absorption coeff " + str(
            -np.log10(edge_emit_bottom / numRays) / float(LSCdimZ)) + "\n")
...
STLfile = '/Users/aakashsjoshi/Downloads/Control.stl'
...
rotateY = False
rotateX = False
saveFileName = 'Controltestresults'
dataFile = ''
if(saveFolder != ''):
