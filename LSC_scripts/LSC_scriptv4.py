    ...
    LSC = Node(
        ...
        Mesh(
            ...
            material = Material(
                refractive_index = 1.5,
                # components = [
                #     Absorber(coefficient = 0.000000000000000000000001),
                #     ]
            ),
        ),
        parent = world
...
def doRayTracing(numRays):
    ...
    vis = MeshcatRenderer(open_browser=dispSim, transparency=False, opacity=0.5, wireframe=True)
    scene = Scene(world)
    vis.render(scene)
...
def analyzeResults(entrance_rays, exit_rays, exit_norms):
    ...
    for k in exit_norms:
        if k[2] != None:
            if ((rotateY or rotateX) is False or enclosingBox):
                if abs(k[2]) <= 0.5:
                    edge_emit += 1
                if abs(k[0] - -1) < 0.1:
                    edge_emit_left += 1
    ...
    numRaysSim = len(exit_norms)
    print("\n Optical efficiency: " + str(edge_emit / numRaysSim) + "\n")
    print("\t\tLeft \tRight \tFront \tBack \n")
    print("Edge emission\t" + str(edge_emit_left / numRaysSim) + " \t" + str(edge_emit_right / numRaysSim) + " \t" + str(
        edge_emit_front / numRaysSim) + " \t" + str(edge_emit_back / numRaysSim) + " \n")
    print("Bottom emission\t" + str(edge_emit_bottom / numRaysSim) + "\t Absorption coeff " + str(
        -np.log10(edge_emit_bottom / numRaysSim) / float(LSCdimZ)) + "\n")
    print("Top emission\t" + str(edge_emit_top / numRaysSim) + "\n")
    resultsFile.write(str(edge_emit_bottom / numRaysSim) + '\n')
    if (saveFileName != ''):
        dataFile.write("Opt eff\t" + str(edge_emit / numRaysSim) + "\n")
        dataFile.write("\t\tLeft \tRight \tFront \tBack \n")
        dataFile.write(
            "Edge emission\t" + str(edge_emit_left / numRaysSim) + " \t" + str(edge_emit_right / numRaysSim) + " \t" + str(
                edge_emit_front / numRaysSim) + " \t" + str(edge_emit_back / numRaysSim) + " \n")
        dataFile.write("type\tposx\tposy\tposz\tdirx\tdiry\tdirz\tsurfx\tsurfy\tsurfz\twav\n")
        dataFile.write("Bottom emission\t" + str(edge_emit_bottom / numRaysSim) + "\t Absorption coeff " + str(
            -np.log10(edge_emit_bottom / numRaysSim) / float(LSCdimZ)) + "\n")
        dataFile.write("Top emission\t" + str(edge_emit_top / numRaysSim) + "\n")
        for ray in entrance_rays:
            dataFile.write("entrance\t")
            for k in range(3):
                dataFile.write(str(ray.position[k]) + "\t")
    ...
    if (saveFolder != ''):
        plt.savefig(saveFolder + "/" + "plots.png", dpi=figDPI)
...
###
resultsFile = open('/Users/aakashsjoshi/Documents/New Results of PV Trace/AllResults.txt','a')

shapes = ['Pyramid','Cone','Spheres','Cylinder']
for shape in shapes:
    shapeFolder = '/Users/aakashsjoshi/Downloads/'+shape+' CAD'
    for file in os.listdir(shapeFolder):
        if shape not in file:
            continue
        print(file, shapeFolder+'/'+file)
        filename = file.split('.')[0]
        STLfile = shapeFolder+'/'+file
        for angle in [0, 30, 45, 60]:
            if angle == 0:
                lightDirection = (0,0,1)
            if angle == 30:
                lightDirection = (1 / 2, 0, np.sqrt(3) / 2)
            if angle == 45:
                lightDirection = (0, 1/np.sqrt(2), 1/np.sqrt(2))
            if angle == 60:
                lightDirection = (np.sqrt(3)/2, 0, 1/2)
            saveFolder = '/Users/aakashsjoshi/Documents/New Results of PV Trace/'+str(angle)+'degree/'+filename
            if not os.path.exists(saveFolder):
                if not os.path.exists('/Users/aakashsjoshi/Documents/New Results of PV Trace/'+str(angle)+'degree/'):
                    os.mkdir('/Users/aakashsjoshi/Documents/New Results of PV Trace/'+str(angle)+'degree/')
                os.mkdir('/Users/aakashsjoshi/Documents/New Results of PV Trace/'+str(angle)+'degree/'+filename)
            saveFileName = filename+'_'+str(angle)+'degree_Results'
            if os.path.exists(saveFolder+'/'+'plots.png'):
                continue
            resultsFile.write(filename + '\t' + str(angle)+'degree' + '\t')
